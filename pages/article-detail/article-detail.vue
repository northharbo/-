<template>
	<view class="content">
		...
		<!-- v-if="weChatArticleUrl" -->
		<view class="loading-emty" v-if="is_show_login">
			<view  class="data-login-flex">
			...
			<scroll-view scroll-y="true" class="scroll-Y">
				<view class="scroll-main">
							...
							<view class="user">
								...
								<view class="artcile-info">
									...
									<view class="visit">
										...
										<text class="des-count">{{articleInfo.views || 0}}</text>
									</view>
								</view>
							</view>
						</view>
					...
					<view class="des-content">
						<mp-html :content="content" selectable ></mp-html>
					...
					<view class="detail-btn">
						<view class="dt-btn-lf">
							<image class="icon-img"
								...
								mode=""></image>
							<text class="view-text">{{articleDetail.views || 0}}</text>
						</view>
		...
		<view class="btm-btn" v-if="!is_show_login">
...
<script>
	...
	import {htmlAddStyle} from '../../utils/edit/htmlAddStyle.js'
	import {
		mapState,
		mapActions
	} from 'vuex'
	export default {
		...
		onShareAppMessage() {
			...
			return {
				title: this.title,
				imageUrl: this.articleInfo.imageUrl,
				path: `/pagesShare/article-share/article-share?id=${this.articleId}&classId=${this.classId}`,
			};
		},
		onLoad(e) {
			...
			if(e.type){
				...
				console.log(1, e)
				this.articleInfo = e
				// this.content = marked(article[this.articleInfo.id] || "")
				this.title = this.articleInfo.title
				this.articleId = this.articleInfo.id
				this.classId = this.articleInfo.classId
		...
		methods: {
			...
			async getArticleDetail() {
				...
				this.content = htmlAddStyle(marked(res.content))
				//  文章详情所有信息
				this.articleDetail = res;
