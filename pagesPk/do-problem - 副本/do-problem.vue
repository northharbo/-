			...
			<view class="mainBox radius shadow-warp bg-white margin-top relative">
				...
				<view v-show="hasData">
					...
					<view class="margin-top cu-progress radius striped active">
						<view class="bg-blue" :style="[{ width: loading ? progress + '%' : '' }]">{{  progress.toFixed(2)}}%</view>
					</view>
			...
			<view class="warp">
				<view class="rect" @tap.stop>
					...
					<view style="width: 350rpx; margin: 50rpx auto 0;">
						<!-- <button v-if="isShare != 1" open-type="share" class="cu-btn round bg-blue shadow" style="width: 100%;">分享获得10个积分</button> -->
						<button v-if="isShare == 1" open-type="share" class="cu-btn round bg-blue shadow" style="width: 100%;">分享给好友,一起学习哦</button>
					</view>
				</view>
			</view>
		</u-mask>
		...
		data() {
			return {
				...
				DataList: [{
						serialNumber: '1',
						name: '',
						title: '下列元素中，为行内元素的是为行内元素的是为行内元素的是为行内元素的是为行内元素的是为行内元素的是（ ）',
						itemList: [{
								number: 'A',
								answer: 'divdivdivdivdivdivdivdivdivdivdivdivdivdivdivdivdivdivdivdivdivdivdivdivdivdiv'
							},
							{
								number: 'B',
								answer: 'span'
